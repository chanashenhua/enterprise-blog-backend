...
@Component
public class TagAuditOutboxDispatcher {
    private final JdbcTemplate jdbcTemplate;
    private final TagAuditClient auditClient;
    private final int batchSize;
    private final int maxRetries;

    public TagAuditOutboxDispatcher(
            JdbcTemplate jdbcTemplate,
            TagAuditClient auditClient,
    ...
    public void deliverPending() {
        ...
        for (Map<String, Object> event : events) {
            ...
            try {
                ...
                jdbcTemplate.update(
                        "update tag_audit_event set status = 'DELIVERED', updated_at = current_timestamp where id = ? and status = 'PENDING'",
                        eventId
                );
            } catch (Exception ex) {
                jdbcTemplate.update(
                        """
                                update tag_audit_event
