...
@Component
public class ReviewAuditOutboxDispatcher {
    private final JdbcTemplate jdbcTemplate;
    private final ReviewAuditClient auditClient;
    private final int batchSize;
    private final int maxRetries;

    public ReviewAuditOutboxDispatcher(
            JdbcTemplate jdbcTemplate,
            ReviewAuditClient auditClient,
    ...
    public void deliverPending() {
        ...
        for (Map<String, Object> event : events) {
            ...
            try {
                ...
                jdbcTemplate.update(
                        "update review_audit_event set status = 'DELIVERED', updated_at = current_timestamp where id = ? and status = 'PENDING'",
                        eventId
                );
            } catch (Exception ex) {
                jdbcTemplate.update(
                        """
                                update review_audit_event
